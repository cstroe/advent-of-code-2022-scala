...
object Day16Part2 {
  case class Valve(name: String, flowRate: Long, connectedTo: List[String]) {
    override def toString: String =
      s"""$name --- $flowRate ----> ${connectedTo.mkString(",")}"""
  }
  ...
  def parseInput(fileName: String): Map[String, Valve] = {
    ...
    readFileToLines(s"src/main/resources/day16/$fileName")
      .map {
  ...
  def expandPath(
    ...
    val elephantSelfHops: List[Hop] =
      if (!openValves.contains(elephantLocation)) {
        List(Hop(elephantLocation, Open))
      } else {
        List.empty
      }

    val yourNewHopsWithPressure =
      (selfHops ++ valves(yourLocation).connectedTo.map { adjacentValve =>
        ...
        if (
          possibleHop.status == Open && !openValves.contains(possibleHop.valve)
        ) {
    ...
    val elephantHopsWithPressure: List[(List[Hop], Long)] =
      (elephantSelfHops ++ valves(elephantLocation).connectedTo.map {
        adjacentValve =>
          if (openValves.contains(adjacentValve)) {
            Hop(adjacentValve, Open)
          } else {
    ...
    } yield { (yourHop._1, elephantHop._1, yourHop._2 + elephantHop._2) }
    ...
    newHops
      .filter { case (yourPath, elephantPath, _) =>
        ...
        if (
          yourLastHop.valve == elephantLastHop.valve && yourLastHop.status == Open && elephantLastHop.status == Open
        ) {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val startingPath = Path(
      yourHops = List(Hop("AA", Closed)),
      elephantHops = List(Hop("AA", Closed)),
      0
    )
    val numMinutes = 26
    val paths = (1 to numMinutes).foldLeft(List(startingPath)) {
      case (paths, minute) =>
