...
object Day15Part1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data = readFileToLines("src/main/resources/day15/testinput").map {
    ...
    val sensorData = data.map { case (sensor, _, distance) => (sensor, distance)}
    val minX = sensorData.map { case (sensor, distance) => sensor.x - distance }.min
    val maxX = sensorData.map { case (sensor, distance) => sensor.y + distance }.max

    println(s"minX = $minX, maxX = $maxX")

    val rowsWhereCannotBe: Int = (minX to maxX).map(Point(_, row)).map { test =>
      if (test.x % 10000 == 0) { println(s"Testing $test")}
      if (sensorData.exists { case (sensor, distance) => isWithinPoint(test, sensor, distance) }) {
