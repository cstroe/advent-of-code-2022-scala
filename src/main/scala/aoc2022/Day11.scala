  ...
  def runRound(roundNum: Int, monkeys: List[Monkey]) = {
    ...
    monkeys.foreach { monkey =>
      ...
      monkey.items.foreach { item =>
        ...
        println(
          s"    Monkey gets bored with item. Worry level is divided by 3 to $boredWorryLevel."
        )
        if (monkey.test(boredWorryLevel)) {
          println(
            s"(T) Item with worry level $boredWorryLevel is thrown to monkey ${monkey.onTrueThrowTo}."
          )
          monkeys(monkey.onTrueThrowTo).items.addOne(boredWorryLevel)
        } else {
          println(
            s"(F) Item with worry level $boredWorryLevel is thrown to monkey ${monkey.onFalseThrowTo}."
          )
  ...
  def main(args: Array[String]): Unit = {
    ...
    val monkeys = contents
      ...
      .map { monkeySpec =>
        ...
        val monkeyItems =
          specLines(1).split(":")(1).split(",").map(_.trim.toInt).toList
        val operation =
          toOperation(specLines(2).split(":")(1).split("=")(1).trim())
...
object Day11Part2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val upperLimit: BigInt =
      monkeys.map(_.test.n).foldLeft(BigInt(1)) { case (acc, n) => acc * n }
    ...
    (1 to 10000).foreach { round =>
      ...
      while (run) {
        monkeys.flatMap(_.items).find(w => w < upperLimit) match {
          case Some(_) =>
            println("Not at upper limit yet")
            run = false
          case None =>
    ...
    val topMonkeys: List[BigInt] = monkeys
      .map { monkey =>
        monkey.inspections.get()
      }
      .sorted
